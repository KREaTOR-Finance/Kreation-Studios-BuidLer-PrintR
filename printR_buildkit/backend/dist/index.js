...
if (!databaseUrl && dbPath && receipts.migrate)
    receipts.migrate();
...
if (!databaseUrl && dbPath && analytics.migrate)
    analytics.migrate();
...
if (developerLeads.migrate)
    developerLeads.migrate();
app.use(cors({
...
const playerState = new Map(); // sessionId -> playerId -> state
function getPlayerState(sessionId, playerId) {
    const smap = playerState.get(sessionId) ?? new Map();
    playerState.set(sessionId, smap);
    const ps = smap.get(playerId) ?? { markersRemaining: 10, scoreRealized: 1000 };
    smap.set(playerId, ps);
    return ps;
}
function buildHud(session, playerId, ps) {
...
function onClientEvent(playerId, ev) {
    ...
    if (ev.intent === "CLOSE") {
        ...
        if (!pos)
            return;
        if ((s.tickIndex - pos.entryTickIndex) < CONST.MIN_HOLD_TICKS)
            return;
...
function resetSessions() {
    sessions.clear();
    playerState.clear();
    const archetypes = ["STANDARD", "THIN_FLOAT", "DEEP_POOL", "WHIPSAW"];
