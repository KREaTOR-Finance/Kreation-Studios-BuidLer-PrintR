...
function derivePhase(session, now) {
    if (now >= session.endTimeMs)
        return "ENDED";
    if (now >= session.closingTimeMs)
        return "CLOSING";
    if (now >= session.closingTimeMs - 30_000)
        return "FINAL_COMMIT_WARNING";
    return "LIVE";
}
export function registerSessionRoutes(app, args) {
