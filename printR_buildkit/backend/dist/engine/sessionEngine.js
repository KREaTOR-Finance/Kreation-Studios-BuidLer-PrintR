...
export async function stepTick(session, deps) {
    const now = Date.now();
    const phase = now >= session.endTimeMs ? "ENDED" :
    ...
    const positions = session.positions.map(p => {
        if (!p.isOpen)
            return p;
        if ((nextTickIndex - p.entryTickIndex) < CONST.MIN_HOLD_TICKS)
            return p;
        if (breakPointTriggered(p, price))
            return { ...p, isOpen: false, wasBreakPoint: true };
        return p;
    });
    const next = {
        ...session,
