...
export class PostgresRoundsStore {
    pool;
    ...
    async getRound(roundId) {
        const { rows } = await this.pool.query("select * from round_history where round_id = $1 limit 1", [roundId]);
    ...
    async recordRound(round) {
        ...
        try {
            await client.query("begin");
            await client.query("insert into player_profiles (player_ref) values ($1) on conflict (player_ref) do nothing", [round.playerRef]);
            const existing = await client.query("select * from round_history where round_id = $1 limit 1", [round.roundId]);
            if (existing.rowCount && existing.rowCount > 0) {
                const profileRes = await client.query("select player_ref, points, streak, total_rounds, updated_at from player_profiles where player_ref = $1", [round.playerRef]);
                await client.query("commit");
                return { round: mapRound(existing.rows[0]), profile: mapProfile(profileRes.rows[0], round.playerRef), created: false };
            }
            await client.query(`insert into round_history
         ...
         set points = greatest(0, points + $2),
             ...
             updated_at = now()
         where player_ref = $1
         returning player_ref, points, streak, total_rounds, updated_at`, [round.playerRef, round.pointsDelta, round.streakDelta]);
            await client.query("commit");
    ...
    async listRounds(playerRef, limit = 50) {
        const { rows } = await this.pool.query(`select * from round_history
       where player_ref = $1
