...
export class SqliteRoundsStore {
    db;
    constructor(dbPath) {
    ...
    async recordRound(round) {
      ...
      `).get(round.roundId);
        ...
        VALUES
          (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(round.roundId, round.playerRef, round.sessionId ?? null, round.tier, round.direction, round.status, round.pointsDelta, round.streakDelta, round.randomnessHex, round.z, round.requestId, round.nonce, round.signature, round.proofRef ?? null, round.proof ? JSON.stringify(round.proof) : null, round.requestedAt, round.fulfilledAt);
            this.db.prepare(`
        UPDATE player_profiles
