 ...
 */
export class SqliteCreditsStore {
    db;
    constructor(dbPath) {
    ...
    async getBalance(playerRef) {
        const row = this.db.prepare("SELECT sessions_balance FROM credits_balances WHERE player_ref = ?").get(playerRef);
    ...
    async addLedger(entry) {
        ...
        VALUES
          (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(full.id, full.playerRef, full.deltaSessions, full.reason, full.source, full.externalRef ?? null, full.idempotencyKey ?? null, full.meta ? JSON.stringify(full.meta) : null, full.createdAt);
            const prev = this.db.prepare("SELECT sessions_balance FROM credits_balances WHERE player_ref = ?").get(full.playerRef);
    ...
    async consumeOneSession(playerRef, sessionId, idempotencyKey = "") {
        ...
        `).get(playerRef, sessionId, idempotencyKey);
            ...
            const prev = this.db.prepare("SELECT sessions_balance FROM credits_balances WHERE player_ref = ?").get(playerRef);
            const prevBal = prev ? Number(prev.sessions_balance) : 0;
            if (prevBal < 1)
        ...
        VALUES
          (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(randomUUID(), playerRef, -1, "consume", "system", sessionId, idempotencyKey || null, idempotencyKey ? JSON.stringify({ idempotencyKey }) : null, now);
            if (prev) {
                this.db.prepare("UPDATE credits_balances SET sessions_balance = ?, updated_at_ms = ? WHERE player_ref = ?")
