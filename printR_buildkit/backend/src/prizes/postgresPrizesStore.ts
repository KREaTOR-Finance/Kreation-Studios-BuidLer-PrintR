...
export class PostgresPrizesStore implements PrizesStore {
  private pool: Pool;
  ...
  async listClaims(playerRef: string): Promise<PrizeClaim[]> {
    const { rows } = await this.pool.query(
      `select id, player_ref, prize_id, cost_points, status, created_at, meta_json
       from prize_claims
       where player_ref = $1
  ...
  async claimPrize(playerRef: string, prizeId: string, costPoints: number): Promise<{ claim: PrizeClaim; profile: PlayerProfile }> {
    ...
    try {
      ...
      const existing = await client.query(
        "select * from prize_claims where player_ref = $1 and prize_id = $2 limit 1",
        [playerRef, prizeId]
      );
      ...
      const balRes = await client.query(
        "select points from player_profiles where player_ref = $1 for update",
        [playerRef]
      );
