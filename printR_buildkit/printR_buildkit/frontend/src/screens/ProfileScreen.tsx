...
export function ProfileScreen({ game }:{ game:GameAPI }){
  ...
  const h = useHaptics();
  const playerRef = (state as any).playerRef ?? "tg:demo";
  const walletPubkey = (state as any).walletPubkey ?? null;
  const { sessionsBalance, refresh } = useCreditsBalance({ playerRef, walletPubkey });
    ...
    <div style={{ display:"flex", flexDirection:"column", gap: 14 }}>
      ...
      <Card title="Sessions Balance">
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <button onClick={()=>{ play("tap"); h.tap(); refresh(); }} style={{ padding:"10px 14px", borderRadius: 12, background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.14)", color:"white" }}>
            Refresh
          </button>
        </div>
      ...
      <Card>
        ...
        <div style={{marginTop:16, display:"grid", gridTemplateColumns:"1fr 1fr", gap: 10}}>
          ...
          <div style={statBox()}>Direction<br/><b>{state.selection.direction}</b></div>
        </div>
