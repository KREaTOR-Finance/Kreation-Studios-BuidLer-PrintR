...
export function DirectionSelector({ value, onChange }: { value: Direction; onChange: (d: Direction)=>void }){
  ...
  return (
    <div style={{ display:"flex", gap: 12 }}>
      <button style={btn(value==="UP","rgba(34,197,94,0.25)")} onClick={()=>{h.selection(); onChange("UP");}}>UP</button>
      <button style={btn(value==="DOWN","rgba(239,68,68,0.22)")} onClick={()=>{h.selection(); onChange("DOWN");}}>DOWN</button>
    </div>
  );
}
