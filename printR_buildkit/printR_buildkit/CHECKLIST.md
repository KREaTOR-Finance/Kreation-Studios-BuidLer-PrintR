...
- [ ] Wire ORAO or Switchboard SDK + accounts into adapter
- [ ] Proof links / explorers for settlement receipts (next)

## F) Core Game Logic (Not finished yet — next phase)
We still need:
- [ ] Deterministic mapping from randomness → outcome (no Math.random)
- [ ] Anti-cheat + replay protection (roundId, nonce, signature)
- [ ] Tier balancing math (engagement, fairness, economics)
- [ ] Round history persistence (server / on-chain event index)
- [ ] Prize redemption rules + fulfillment flows
- [ ] Rate limits + abuse prevention
- [ ] Analytics events (retention, funnels, streak behavior)
...
- [x] Backend endpoints scaffold:
  ...
  - [ ] POST `/api/payments/stripe/checkout` (needs Stripe SDK)
  - [ ] POST `/webhooks/stripe` (needs signature verification + event parse)
- [ ] Persist credit balances + ledger in DB (schema provided in `design_docs/16_Payments_and_Credits/CREDITS_DB_SCHEMA.sql`)
- [ ] Store screen wiring (Stripe + USDC purchase)
- [ ] Unified receipt/ledger for credits and prize claims

## H) Program-Enforced Settlement (Solana)
- [x] Anchor program scaffold added (`/onchain/anchor/programs/printr`)
- [ ] Wire Switchboard CPI in `request_round`
- [ ] Harden callback authorization in `settle_round`
...
- Credits authoritative in SQLite (DATABASE_PATH).
