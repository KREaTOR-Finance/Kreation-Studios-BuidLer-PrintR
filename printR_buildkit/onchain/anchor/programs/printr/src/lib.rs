...
pub mod printr {
    ...
    pub fn settle_round(ctx: Context<SettleRound>, result: Vec<u8>) -> Result<()> {
        ...
        require_keys_eq!(ctx.accounts.player_profile.authority, round.player, PrintRErr::BadPlayerProfile);
        ...
        // Validate randomness service accounts are owned by SRS
        let (expected_state, _bump) = Pubkey::find_program_address(&[b"STATE"], &SolanaRandomnessServiceID);
        require_keys_eq!(ctx.accounts.randomness_state.key(), expected_state, PrintRErr::BadRandomnessState);
        require_keys_eq!(*ctx.accounts.randomness_state.to_account_info().owner, SolanaRandomnessServiceID, PrintRErr::BadRandomnessState);
        require_keys_eq!(*ctx.accounts.randomness_request.to_account_info().owner, SolanaRandomnessServiceID, PrintRErr::BadRandomnessRequestOwner);
        ...
        emit!(RoundSettled {
            ...
            points_delta,
            streak_delta,
            fulfilled_at: round.fulfilled_at,
            sb_request: round.sb_request,
        });
...
pub enum PrintRErr {
    ...
    #[msg("Player profile does not match round")]
    BadPlayerProfile,
    #[msg("Bad randomness state account")]
    BadRandomnessState,
    #[msg("Bad randomness request owner")]
    BadRandomnessRequestOwner,
}
