...
import { VRF_MODE, SESSION_FLOW } from "../config/env";

const useSim = VRF_MODE === "SIM";
const useSessionFlow = SESSION_FLOW !== "ROUND";
const vrf = useSim
  ? SimVrfAdapter
...
export function useGameMachine() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const player = await getPlayerHeaders();
        const profile = await apiGet<{ ok: boolean; points: number; streak: number }>("/api/profile", player);
        ...
        logError("useGameMachine/profileSync", err);
      }
    })();
  }, []);
  ...
  const playRound = async (sessionId?: string): Promise<JoinOutcome> => {
    ...
    try {
      ...
      const resolvedSessionId = String(joinJson?.sessionId ?? joinTargetId);
      playToken = typeof joinJson?.playToken === "string" ? joinJson.playToken : "";

      join = { ok: true, mode, phase, sessionId: resolvedSessionId };
      dispatch({ type: "JOIN_UPDATED", join: { mode, phase, sessionId: resolvedSessionId } });

      if (mode === "spectate") {
        dispatch({ type: "NAVIGATE", view: "live" });
        inFlightRef.current = false;
        return { ok: true, mode, phase, sessionId: resolvedSessionId };
      }
